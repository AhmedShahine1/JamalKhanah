...
public class WorkHoursController : BaseApiController, IActionFilter
{
    ...
    // POST: api/WorkHours
    [HttpPost]
    public async Task<ActionResult<BaseResponse>> AddWorkHour([FromHeader] string lang, WorkHourDto workHour)
    {
        if (_user == null)
        {
            _baseResponse.ErrorCode = (int)Errors.TheUserNotExistOrDeleted;
        ...
        {
           Day = workHour.Day,
           From = new TimeSpan(TimeOnly.FromDateTime(workHour.From).Ticks),
           To = new TimeSpan(TimeOnly.FromDateTime(workHour.To).Ticks),
            MoreData = workHour.MoreData,
            UserId = _user.Id,
            IsDeleted = false,
            IsUpdated = false
        };
