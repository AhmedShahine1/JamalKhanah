...
public class OrderUsersController : BaseApiController, IActionFilter
{
    ...
    public async Task<ActionResult<BaseResponse>> GetOrders([FromHeader] string lang)
    {
        ...
        var orders1 = await _unitOfWork.Orders.FindByQuery(
        criteria: s => s.UserId == _user.Id &&
                       s.IsDeleted == false && s.IsPaid == false)
    .Select(s => new
    {
        s.Id,
        s.CreatedOn,
        ...
        var orders2 = await _unitOfWork.Orders.FindByQuery(
        criteria: s => s.UserId == _user.Id &&
                       s.IsDeleted == false && s.IsPaid == true)
    .Select(s => new
    {
        s.Id,
        s.CreatedOn,
        ...
        var orders = orders1.Concat(orders2).ToList();
